...
def expressivity_with_histogram(graph_state_circuit: GraphStateCircuit, n_samples :int =10000, n_bins: int=1000, method :str='KL'):
    ...
    digitized_data, edges_hist = np.histogram(samples_from_circuit, bins=n_bins, density=True, range=(0,1))
                    ...
                    if sanity_check:
                        for n in g:
                            if not are_lc_equivalent(n, curr_g)[0]:
                                raise RuntimeError("Graph not lc equivalent with all group")
            if not added:
